...
export const getAllProducts = async (req, res) => {
  try {
    const products = await Product.find({}); //leaving empty = find all products
    res.json({ products });
  } catch (error) {
...
export const getFeaturedProducts = async (req, res) => {
  //featured could be stored in redis
  try {
    ...
    //store in redis from mongodb(featuredProducts) for future quick access
...
export const deleteProduct = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id); //parametro da rota(/:id)
    if (!product) {
      return res.status(404).json({ message: "Products not found" });
    }

    if (product.image) {
      //this gets the id of the image in cloudinary. This code is from the cloudinary API
