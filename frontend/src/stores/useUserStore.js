...
axios.interceptors.response.use(
  //if nothing is wrong, just continue
  (response) => response,
  //if there's error, try this
  async (error) => {
    const originalRequest = error.config;
    //if user is unauthorrized and it's the first time he's trying to do a request...
    if (error.response?.status === 400 && !originalRequest._retry) {
      ...
      try {
        ...
        if (refreshPromise) {
          ...
          //try again what the user was trying to do unauthorized
